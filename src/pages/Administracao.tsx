...
export default function Administracao() {
    ...
    // Turnos (alterações 8 e 9).
    const [usesShifts, setUsesShifts] = useState(false);
    const [aGuardarTurnoToggle, setAGuardarTurnoToggle] = useState(false);
    const [turnos, setTurnos] = useState<Turno[]>([]);
    const [nomeNovoTurno, setNomeNovoTurno] = useState("");
    const [inicioNovoTurno, setInicioNovoTurno] = useState("");
    const [fimNovoTurno, setFimNovoTurno] = useState("");
    const [intervaloInicioNovoTurno, setIntervaloInicioNovoTurno] = useState("");
    const [intervaloFimNovoTurno, setIntervaloFimNovoTurno] = useState("");
    const [erroTurno, setErroTurno] = useState("");
    const [aCriarTurno, setACriarTurno] = useState(false);

    const carregarTurnos = () => {
        api.get("/companies/shifts").then((r) => setTurnos(r.data || [])).catch(() => setTurnos([]));
    };

    const alternarTurnos = async (ligar: boolean) => {
        setAGuardarTurnoToggle(true);
        try {
            await api.put("/admin/company/shifts", { uses_shifts: ligar });
            setUsesShifts(ligar);
        } catch {
            setErro("Não foi possível alterar o regime de turnos.");
        } finally {
            setAGuardarTurnoToggle(false);
        }
    };

    const criarTurno = async () => {
        setErroTurno("");
        if (!nomeNovoTurno.trim() || !inicioNovoTurno || !fimNovoTurno) {
            setErroTurno("Indique o nome, a hora de início e a hora de fim do turno.");
            return;
        }
        setACriarTurno(true);
        try {
            await api.post("/companies/shifts", {
                name: nomeNovoTurno,
                start_time: inicioNovoTurno,
                end_time: fimNovoTurno,
                break_start: intervaloInicioNovoTurno || null,
                break_end: intervaloFimNovoTurno || null,
            });
            setNomeNovoTurno(""); setInicioNovoTurno(""); setFimNovoTurno("");
            setIntervaloInicioNovoTurno(""); setIntervaloFimNovoTurno("");
            carregarTurnos();
        } catch (e: any) {
            setErroTurno(e.response?.data?.detail || "Não foi possível criar o turno.");
        } finally {
            setACriarTurno(false);
        }
    };

    const desativarTurno = async (id: number) => {
        try {
            await api.delete(`/companies/shifts/${id}`);
            carregarTurnos();
        } catch {
            setErroTurno("Não foi possível desactivar o turno.");
        }
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                ...
                <Cartao>
                    <div className="flex items-center justify-between mb-2">
                        <h3 className="text-[14.5px] m-0">Turnos</h3>
                        <label className="flex items-center gap-2 text-[12px] text-dim cursor-pointer select-none">
                            <input
                                type="checkbox"
                                checked={usesShifts}
                                disabled={aGuardarTurnoToggle}
                                onChange={(e) => alternarTurnos(e.target.checked)}
                            />
                            A empresa trabalha por turnos
                        </label>
                    </div>
                    <p className="text-[11.5px] text-dim mb-3">
                        Com o regime de turnos ligado, os colaboradores podem ser associados a um dos turnos abaixo
                        na respectiva ficha, em vez de um horário fixo.
                    </p>

                    {turnos.length > 0 && (
                        <div className="space-y-1.5 mb-3">
                            {turnos.map((t) => (
                                <div key={t.id} className={`flex items-center justify-between px-3 py-2 border border-line rounded-lg text-[12.5px] ${!t.is_active ? "opacity-50" : ""}`}>
                                    <span>
                                        <b className="text-strong">{t.name}</b>
                                        <span className="text-dim ml-2">{t.start_time}–{t.end_time}</span>
                                        {t.break_start && t.break_end && (
                                            <span className="text-dim ml-2">(intervalo {t.break_start}–{t.break_end})</span>
                                        )}
                                        {!t.is_active && <span className="text-dim ml-2">— desactivado</span>}
                                    </span>
                                    {t.is_active && (
                                        <button
                                            type="button"
                                            onClick={() => desativarTurno(t.id)}
                                            className="text-[11px] text-dim hover:text-bad transition-colors"
                                        >
                                            Desactivar
                                        </button>
                                    )}
                    ...
                    <div className="grid grid-cols-2 gap-x-3">
                        <div className="col-span-2">
                            <label className="block text-[10.5px] uppercase tracking-wide text-dim mb-1">Nome do turno</label>
                            <input value={nomeNovoTurno} onChange={(e) => setNomeNovoTurno(e.target.value)} placeholder="Ex.: Manhã" className={campoCls} />
                        </div>
                        <div>
                            <label className="block text-[10.5px] uppercase tracking-wide text-dim mb-1">Início</label>
                            <input type="time" value={inicioNovoTurno} onChange={(e) => setInicioNovoTurno(e.target.value)} className={campoCls} />
                        </div>
                        <div>
                            <label className="block text-[10.5px] uppercase tracking-wide text-dim mb-1">Fim</label>
                            <input type="time" value={fimNovoTurno} onChange={(e) => setFimNovoTurno(e.target.value)} className={campoCls} />
                        </div>
                        <div>
                            <label className="block text-[10.5px] uppercase tracking-wide text-dim mb-1">Início do intervalo (opcional)</label>
                            <input type="time" value={intervaloInicioNovoTurno} onChange={(e) => setIntervaloInicioNovoTurno(e.target.value)} className={campoCls} />
                        </div>
                        <div>
                            <label className="block text-[10.5px] uppercase tracking-wide text-dim mb-1">Fim do intervalo (opcional)</label>
                            <input type="time" value={intervaloFimNovoTurno} onChange={(e) => setIntervaloFimNovoTurno(e.target.value)} className={campoCls} />
                        </div>
                    </div>
                    {erroTurno && <Notice variante="alert" className="mb-3">{erroTurno}</Notice>}
                    <Botao onClick={criarTurno} disabled={aCriarTurno}>
                        {aCriarTurno ? "A criar..." : "Adicionar turno"}
                    </Botao>
                </Cartao>
            </div>
