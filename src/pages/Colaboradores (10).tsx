...
// Sugestões para os campos combobox (dropdown + escrever livre).
// O backend guarda como texto livre, por isso o utilizador pode escolher ou escrever.
const CARGOS_SUGERIDOS = [
    "Diretor", "Gestor", "Chefe de Departamento", "Coordenador", "Supervisor",
    "Analista", "Técnico", "Especialista", "Assistente", "Administrativo",
    "Operador", "Estagiário",
];
const CATEGORIAS_SUGERIDAS = ["Júnior", "Pleno", "Sénior", "Especialista"];
...
                                            <button type="button" onClick={() => navigate(`/colaboradores/${c.id}/portal`, { state: { nome: c.full_name } })} className="text-strong font-bold hover:text-pri hover:underline text-left">{c.full_name}</button>
                                        </td>
                ...
                <div>
                    <div className="grid grid-cols-2 gap-x-3">
                        ...
                        <div><label className="block text-[10.5px] uppercase tracking-wide text-dim mb-1">Categoria</label>
                            <input value={jobCategory} onChange={(e) => setJobCategory(e.target.value)} list="lista-categorias" placeholder="Escolha ou escreva" className={inputCls} />
                            <datalist id="lista-categorias">
                                {CATEGORIAS_SUGERIDAS.map((x) => <option key={x} value={x} />)}
                            </datalist></div>
                        <div><label className="block text-[10.5px] uppercase tracking-wide text-dim mb-1">Cargo</label>
                            <input value={jobTitle} onChange={(e) => setJobTitle(e.target.value)} list="lista-cargos" placeholder="Escolha ou escreva" className={inputCls} />
                            <datalist id="lista-cargos">
                                {CARGOS_SUGERIDOS.map((x) => <option key={x} value={x} />)}
                            </datalist></div>
