...
interface Processo {
    ...
    committee_members?: { id: number; full_name: string }[];
}
                        ...
                        <Cartao key={p.id}>
                            ...
                            {p.committee_members && p.committee_members.length > 0 && (
                                <div className="mt-2 text-[11.5px] text-dim">
                                    ...
                                    {p.committee_members.map((m) => m.full_name).join(", ")}
                                </div>
                            )}
...
function ModalInstaurar({ colaboradores, aoFechar, aoCriar }: {
    ...
    // Comissão disciplinar: 3 membros com perfil Director (alteração 11).
    // Só se pede a escolha se a empresa tiver directores suficientes — não
    // bloqueia a instauração em empresas com menos de 3 directores.
    const directores = colaboradores.filter((c) => c.role === "director" && c.id !== accusedId);
    const exigeComissao = directores.length >= 3;
    const [committeeIds, setCommitteeIds] = useState<number[]>([]);
    const alternarMembro = (id: number) =>
    ...
    const submeter = async () => {
        ...
        setAEnviar(true);
        try {
            const resp = await api.post("/disciplinary", { accused_id: accusedId, reference, imputed_facts: facts });
            if (exigeComissao && committeeIds.length === 3) {
                await api.post(`/disciplinary/${resp.data.id}/committee`, { member_ids: committeeIds });
            }
            aoCriar();
        } catch (err: any) {
    ...
    const podeSubmeter = !!accusedId && !!reference && facts.length >= 3
        && (!exigeComissao || committeeIds.length === 3) && !aEnviar;
            ...
            {exigeComissao && (
                <>
                    <label className="block text-[10.5px] uppercase tracking-wide text-dim mb-1">
                        Comissão disciplinar — escolha 3 directores ({committeeIds.length}/3)
                    </label>
                    <div className="border border-line rounded-lg p-2 mb-4 max-h-[140px] overflow-y-auto space-y-1">
                        {directores.map((d) => (
                            <label key={d.id} className="flex items-center gap-2 text-[12.5px] px-1.5 py-1 rounded hover:bg-panel cursor-pointer">
                                <input
                                    type="checkbox"
                                    checked={committeeIds.includes(d.id)}
                                    disabled={!committeeIds.includes(d.id) && committeeIds.length >= 3}
                                    onChange={() => alternarMembro(d.id)}
                                />
                                {d.full_name}
                            </label>
                        ))}
